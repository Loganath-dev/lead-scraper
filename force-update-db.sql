-- 1. Drop existing old tables to prevent conflicts
...
CREATE POLICY "Users can insert their own searches" ON searches FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view their own saved leads" ON saved_leads FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own saved leads" ON saved_leads FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own saved leads" ON saved_leads FOR DELETE USING (auth.uid() = user_id);

-- 8. Trigger to update profiles on user signup
...
-- 9. Function to deduct credits safely
